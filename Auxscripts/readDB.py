...
def pulse_list(DATABASE_I_path):
	"""
...
def param_list(DATABASE_I_path):
	"""
...
def find_pulse(pulse,DATABASE_I_path):
...
def spect_reconstruct(pulse,DATABASE_I_path):
	"""
	...
	#sub_db_path = str(pulse);

	#sub_db = DATABASE[sub_db_path]
	
	#data = sub_db[str(int(pulse))]
	data 			= DATABASE[str(int(pulse))]
...
def parray(key,index,DATABASE_II_path):
	"""
