...
if 1:  #FILTERS AND FREE PARAMETERS
	...
	time_filter=True
	SS_time_threshold = 75e-3  #Consider modes only of length greater than 75ms
	...
	t_o = 0.1; f_o = 0.05 #Stitches together modes if overlap by 10% in time and 5% in f space. 
...
if 1: #MAIN SETUP
	...
	storing_file = "/home/ht2059/Documents/test.pkl"  #Pkl file worked well for me when storing pd DataFrame
	logs_file = "hi.txt"
	
	image_save = True; #if true, saves images of identified mode line plot to save folder
	image_save_freq = 2;
	save_folder = "/home/ht2059/Documents/plots"


	
	PoIs = ["bvac","ne0","n95","ipla","TE0","s95","NBLM","PTOT",
			"q0","q95","btnm","ka3_ccds","gradn95"] #Parameters of interest. Can add in any that are in Database 1. Averaged over mode time (and ~10^2 ms prior)
	...
	try:
		pulse_counter = int(np.where(pulses==96851)[0][0])
	except:
...
for pulse in pulses[pulse_counter:]: #MAIN LOOP
	...
	if 1: #Removal of indices with f > 500kHz. We have lower confidence in results outside this range (mostly ICRH/noise)
		...
		f_high_bin =MAX_F//df; f_low_bin = MIN_F//df 
		freqs_all = NZS[:,0] #Currently indices as ft
		boolH = freqs_all >= f_high_bin ; boolL = freqs_all <= f_low_bin
			...
			if density_filter==True:
				...
				for sig_str in sig_strs:
					...
					if np.mean(sig_str[:,1])<f_50_bin:
						if index_density(sig_str)<0.05:
							pass
						else:
							sig_strs_dens.append(sig_str)
					else:
						if index_density(sig_str)<0.1:
							pass
						else:
							sig_strs_dens.append(sig_str)
	...
	if 1: #Finding defining properties (t,f,m,A,parameters) of each SS in sig _strs, then storing in datfram
		...
		for SS in SIG_STRS_ALL:
			...
			if 1: #time, frequencies and amplitude
				...
				t_A = t_find(inds_n_SS[1][ind_max])-tStart;f_A = f_find(inds_n_SS[0][ind_max])
			...
			if 1: #PMNS
				...
				m = np.array(ms); m = np.ravel(m); m = list(m) #Mafs
			...
			if 1: #Parameters averaged over time interval
				...
				for param in PoIs:
					...
					for i in TS:
						...
						boolL = time < (t_min-i); boolR = time > t_max
